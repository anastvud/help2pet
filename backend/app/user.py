...
async def register_petsitter_basic(
    petsitter: UserBasicCreate,
    db: AsyncSession = Depends(get_db)
):
    result = await db.execute(select(Petsitter).where(Petsitter.username == petsitter.username))
    if result.scalar_one_or_none():
        raise HTTPException(status_code=400, detail="Username already registered")

    result = await db.execute(select(Petsitter).where(Petsitter.email == petsitter.email))
    if result.scalar_one_or_none():
    ...
    return {"message": "Basic registration successful", "petsitter_id": new_petsitter.id}
...
async def register_owner_basic(
    owner: UserBasicCreate,
    db: AsyncSession = Depends(get_db)
):
